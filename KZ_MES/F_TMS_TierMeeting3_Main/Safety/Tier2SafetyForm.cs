...
{
    public partial class Tier2SafetyForm : MaterialForm
    {
        private int days;
        ...
        public Tier2SafetyForm(string date, string month, string plant, string section)
        {
            this.date = date;
            this.month = month;
            this.plant = plant;
        ...
        private void gridData_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (flag)
            {
                //Tier3SafetyForm frm = new Tier3SafetyForm(date, cbxMonth.Text, this.gridData.Rows[index].Cells["0"].Value.ToString());
                //frm.Show();
            }
        }
