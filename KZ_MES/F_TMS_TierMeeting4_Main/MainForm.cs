    ...
    public partial class MainForm : MaterialForm
    {
        ...
        private void GetDepartmentByUser() {
            ...
            if (Convert.ToBoolean(JsonConvert.DeserializeObject<Dictionary<string, object>>(ret)["IsSuccess"]))
            {
                ...
                dept = dtJson.Rows[0]["STAFF_DEPARTMENT"].ToString();
                txtDepartment.Text = dept;
                if (string.IsNullOrEmpty(dtJson.Rows[0]["STAFF_DEPARTMENT"].ToString())) type = (int) Parameters.QueryDeptType.All;
                if (dtJson.Rows[0]["udf05"].ToString().Equals("0")) type = (int)Parameters.QueryDeptType.Plant;
                else {
                    if (dtJson.Rows[0]["udf07"].ToString().Equals("0")) type = (int)Parameters.QueryDeptType.Section;
                    else type = (int)Parameters.QueryDeptType.Line;
                }
            }
            else
            {
        ...
        private void SetBG()
        {
            ...
            lblTier4Header.ForeColor = textColor;
        ...
        private void InitUI()
        {
            ...
            if (height < 800)
            {
                lblTier4Header.Dock = DockStyle.Left;
            }
            //btnKZAP.Font = new Font("宋体", (float)height / 20f, FontStyle.Regular, GraphicsUnit.Point, 134);
            //btnMA.Font = new Font("宋体", (float)height / 20f, FontStyle.Regular, GraphicsUnit.Point, 134);
            ...
            lblTier4Header.Font = new Font(Parameters.headerFont, height/40f, FontStyle.Bold, GraphicsUnit.Point, 134);
        ...
        {
            KaizenActionForm frm = new KaizenActionForm(date.Text, dept, type);
            frm.Show();
        }
        private void btnMA_Click(object sender, EventArgs e)
        {
            MaturityAssessmentForm frm = new MaturityAssessmentForm(dept, type);
            frm.Show();
        }
        ...
        {
            Tier3DeliveryForm frm = new Tier3DeliveryForm(currentDate.ToString(Parameters.dateFormat));
            frm.Show();
        }
        ...
        private void lblEfficiencyHeader_Click(object sender, EventArgs e)
        {
            ...
            assembly = Assembly.LoadFrom(path + @"\" + "SecondEfficiency_Kanban" + ".dll");
            Type type = assembly.GetType("SecondEfficiency_Kanban.Interface");
            object instance = Activator.CreateInstance(type);
            MethodInfo mi = type.GetMethod("RunCustomize");
            object[] args = new object[2];
            args[0] = Program.client;
            args[1] = date.Value.ToString(Parameters.dateFormat);
        ...
        private void GetQualityData()
        {
            ...
            if (type == (int)Parameters.QueryDeptType.All)
            {
               ret = SJeMES_Framework.WebAPI.WebAPIHelper.Post(Program.client.APIURL, "KZ_QCMAPI",
                "KZ_QCMAPI.Controllers.TierMeetingServer", "Tier4_WeekQuery",
                Program.client.UserToken, JsonConvert.SerializeObject(p));
            }
            else if (type == (int)Parameters.QueryDeptType.Plant)
            {
                p.Add("vPlant", dept);
                ret = SJeMES_Framework.WebAPI.WebAPIHelper.Post(Program.client.APIURL, "KZ_QCMAPI",
        ...
        private void GetDeliveryData()
        {
            ...
            if (Convert.ToBoolean(Newtonsoft.Json.JsonConvert.DeserializeObject<Dictionary<string, object>>(ret)["IsSuccess"]))
            {
                ...
                // dataTable is Total  ok   rate when total =0 show po is going and  otherwise show rate
                DataTable dtJson = SJeMES_Framework.Common.JsonHelper.GetDataTableByJson(json);
                DataTable dtChart = new DataTable();
                dtChart.Columns.Add("RATE");
                dtChart.Columns.Add("DATE");
                for (int i=0;i<dtJson.Rows.Count;i++)
                {
                    DataRow dr = dtChart.NewRow();
                    dr["DATE"] = DateTime.Parse(dtJson.Rows[i]["PLAN_FINISH_DATE"].ToString()).ToString("MM/dd");
                    dr["RATE"] = 0;
                    int total = int.Parse(dtJson.Rows[i]["Total"].ToString());
                    int ok = int.Parse(dtJson.Rows[i]["Ok"].ToString());
                    int intersection = 0;
                    if (!string.IsNullOrEmpty(dtJson.Rows[i]["intersection"].ToString())) {
                        intersection= int.Parse(dtJson.Rows[i]["intersection"].ToString());
                    }
                    ok = ok - intersection;
                    if (total==0)
                    {
                        SetText(txtPOCompletionRate,"PO正在进行中");
                    }
                    else
                    {
                        double rate = (double) (ok* 100 / total);
                        Math.Round(rate, 2);
                        dr["RATE"] = rate;
                        SetText(txtPOCompletionRate, rate.ToString("0.0")+"%");
                        if (rate < 100)
                        {
                            SetBackColor(txtPOCompletionRate, Color.Red);
                        }
                        else
                        {
                            SetBackColor(txtPOCompletionRate, Color.Green);
                        }
                    }
                    dtChart.Rows.Add(dr);
                }
                chartDelivery.Series.Clear();
                chartDelivery.Legends.Clear();
                chartDelivery.Titles.Clear();
                chartDelivery.DataBindTable(dtChart.DefaultView, "DATE");
        ...
        {
            SelectDepartmentForm frm = new SelectDepartmentForm((int) Parameters.QueryDeptType.Plant);
        ...
        private void LoadDeliveryChart()
        {
            ...
            string ret =
                SJeMES_Framework.WebAPI.WebAPIHelper.Post(
                    Program.client.APIURL,
                    "KZ_JMSAPI",
                    "KZ_JMSAPI.Controllers.JMS_GoalAccomplished_ListServer",
                                        "Tier4_PORateQuery",
                    Program.client.UserToken,
                    JsonConvert.SerializeObject(p));
            if (Convert.ToBoolean(JsonConvert.DeserializeObject<Dictionary<string, object>>(ret)["IsSuccess"]))
        ...
        private void lblPOCompletionRate_Click(object sender, EventArgs e)
        {
            PoRateForm frm = new PoRateForm(date.Text);
            frm.Show();
        }
