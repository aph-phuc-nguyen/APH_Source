...
{
    public partial class frmEqMaintain : Form
    {
        ...
        public frmEqMaintain(string machine_no, Dictionary<string, object> OBJ = null)
        {
        ...
        private void SelectData(string machine_no, string flat)
        {
            ...
            try
            {
                ...
                string xml = GDSJ_Framework.Common.WebServiceHelper.RunService(Program.Org, Program.Client.WebServiceUrl, "SJEMS_WBAPI", "SJEMS_WBAPI.MES030", "GetData_BaoYang_PC", p);//
                ...
                else
                {
                    ...
                    string strdt = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(xml, "<MES030A4>", "</MES030A4>");
                    string strold = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(xml, "<oldMES030A4>", "</oldMES030A4>");
                    string strmac001 = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(xml, "<MAC001>", "</MAC001>");
                    ...
                    //MessageBox.Show(palndt.Rows.Count.ToString());
                    ...
                    if (palndt.Rows.Count > 0)
                    {
                        ...
                        //MessageBox.Show(dtmac001.Rows.Count.ToString());
        ...
        private void dgv_MaintainData_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                //string sql = @"select rownum 行号,MAC001A2.item_id 设备编号,value 标准值 from MAC001A1 join MAC001A2 on MAC001A1.item_id=MAC001A2.item_id where machine_no='" + txt_EqNo.Text.Trim() + "' AND MAC001A2.item_id='" + dgv_MaintainData.Rows[e.RowIndex].Cells[2].Value.ToString() + "'";
                string sql = @"select rownum 行号,MAC001A2.item_id 设备编号,value 标准值 from MAC001A1 join MAC001A2 on MAC001A1.item_id=MAC001A2.item_id";
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgv_MaintainData.Rows.Count > 0)
                {
                    ...
                    Dictionary<string, string> p = new Dictionary<string, string>();
                    p.Add("DataTable1", strpalndt);
                    p.Add("DataTable2", strdt);
                    p.Add("EqNo", EqNO);
                    p.Add("EqName", EqName);
                    p.Add("People", People);
                    p.Add("state", state);
                    ...
                    string xml = GDSJ_Framework.Common.WebServiceHelper.RunService(Program.Org, Program.Client.WebServiceUrl, "SJEMS_WBAPI", "SJEMS_WBAPI.MAC040", "Confirm_BaoYang_PC", p);//
                    bool IsSuccess = Convert.ToBoolean(GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(xml, "<IsSuccess>", "</IsSuccess>"));
                    if (!IsSuccess)
                    {
        ...
        private void frmEqMaintain_Load(object sender, EventArgs e)
        {
            LoadMachineData();
        }
        ...
        private void LoadMachineData()
        {
            ...
            //首次进入加载所有设备信息
            string sql = "select machine_no 设备编号,machine_name 设备名称 from MES030M" + strwhere;
        ...
        private void txt_CopyBook_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            //if (!string.IsNullOrEmpty(frmData.ReturnDataXML))
            //{
            //    string manual_name = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(frmData.ReturnDataXML, "<MANUAL_NAME>", "</MANUAL_NAME>");  //手册名称
            //    txt_CopyBook.Text = manual_name;
            //}
        }
