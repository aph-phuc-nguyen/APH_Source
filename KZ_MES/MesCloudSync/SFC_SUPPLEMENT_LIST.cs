...
{
    class SFC_SUPPLEMENT_LIST
    {
        public static void DoWork()
        {
            string tabName = "SFC_SUPPLEMENT_LIST";
            //MES接受数据库信息初始化
                ...
                if (dt.Rows.Count > 0)
                {
                    using (OracleConnection conn = new OracleConnection(conStr))
                    {
                        try
                        {
                            ...
                            //OracleCommand cmd = new OracleCommand("select * from SFC_SUPPLEMENT_LIST_T where 1 = 2", conn);
                            ...
                            {
                                string updataSql = @"update SFC_SUPPLEMENT_LIST set issync = 'Y' where issync = 'N'";
