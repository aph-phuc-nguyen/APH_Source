...
{
    public partial class SDPDeliveryForm : MaterialForm
    {
        private string strPath = Parameters.strPath;
        public SDPDeliveryForm()
        {
            InitializeComponent();
            InitUI();
        }
        ...
        private void GetData()
        {
            ...
            ExcelApp.Range range = excelSheet.get_Range("A43:M52", Missing.Value);
            if (range != null)
            {
                foreach (ExcelApp.Range r in range)
                {
                    ...
                    if (tmpCol == 1)
                        gridData.Rows.Add();
                    gridData.Rows[tmpRow - 43].Cells[tmpCol - 1].Value = str;
                }
            }
        ...
        private void SDPDeliveryForm_Load(object sender, EventArgs e)
        {
            GetData();
        }
