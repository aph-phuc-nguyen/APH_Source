...
====================GETDATA方法调用=================== @END
 ...
 <Data> @END
 @END
 <location_no></location_no>库位资料 @END
 ...
 <RetData> @END
 如果成功：<BASE011M></BASE011M>库位资料 @END
 如果失败：失败报错 @END
 </RetData> @END
 </Return> @END
 ================================================== @END
...
";
        ...
        public static string GETDATA(object OBJ)
        {
            ...
            try
            {
                ...
                #region 接口参数
                string location_no = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(Data, "<location_no>", "</location_no>");

                #endregion

                string where = string.Empty;

                if (location_no.Contains(","))
                {
                    string[] tmpStr = location_no.Split(',');
...
";
                Dictionary<string, object> p = new Dictionary<string, object>();
                    ...
                    {
                        IsSuccess = true;
                        RetData = "<BASE011M>" + dtXML + @"</BASE011M>";
                    }
                
                else
                {
                    RetData = "该库位不存在或者该库位不在计划或单据内";
                }
                #endregion
            }
            catch (Exception ex)
